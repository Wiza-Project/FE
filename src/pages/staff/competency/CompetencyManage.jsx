...
import { Button, ConfirmDialog, Drawer, SkeletonLoader, toast } from '@/components/common';
import {
...
export default function CompetencyManage() {
  ...
  // BE CompetencyResponse → 화면 표시용 형태. 문항 수는 이 API가 주지 않아 미확인(null)으로 두고
  // 표에서 '—'로 표시한다(0으로 표시하면 문항이 있는 역량도 0건처럼 보임).
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[12px] text-[#9AA0A6] mt-0.5">핵심역량 코드·구조 정의</p>
        </div>
        <Button
          onClick={openNew}
            ...
            <table className="w-full border-collapse text-[12px]">
              ...
              <tbody>
                {coresLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-3 py-10 text-center text-[12px] text-[#CF222E]">
                      핵심역량 목록을 불러오지 못했습니다.
                    </td>
                  </tr>
                ) : orderedCores.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-3 py-10 text-center text-[12px] text-[#9AA0A6]">
                  ...
                  <tr
                    key={c.code}
                    className={`border-b border-[#F3F4F6] last:border-0 transition-colors hover:bg-[#FAFAFA] ${!c.active ? 'opacity-50' : ''}`}
                  >
                    <td
