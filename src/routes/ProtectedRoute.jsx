...
import { CONSENT_PATH, hasCompletedCommonConsent } from '@/routes/redirects';

/**
 * 로그인 여부, 공통 필수 약관 동의 여부, 사용자 유형, role_code로 라우트를 막습니다.
 ...
 * @param {boolean} [props.requireCommonConsent=true] 공통 필수 약관 동의를 요구할지.
 *   /consent 자신은 false 로 꺼야 합니다(켜두면 자기 자신으로 무한 리다이렉트).
 *
 * 주의: 프론트의 차단은 UX용이지 보안 수단이 아닙니다.
 *       브라우저에서 우회 가능하므로 실제 권한 검증은 백엔드가 최종 책임집니다.
 */
export default function ProtectedRoute({ allow, allowRole, requireCommonConsent = true }) {
  ...
  if (requireCommonConsent && !hasCompletedCommonConsent(user)) {
    return <Navigate to={CONSENT_PATH} state={{ from: location }} replace />;
  }
